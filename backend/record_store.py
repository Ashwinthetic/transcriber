"""Record store mapping FAISS vector IDs back to original MSMARCO-XI source records.

The Kaggle full-coverage indexer assigns vector ID = local_id = sequential ordinal
of the source row inside <prefix>train.parquet (see notebooks/MSMARCO_XI_FULL_COVERAGE_HINDI_KAGGLE.ipynb).
Every source record (query, answer, all passages, selection flags) was preserved in
compressed parquet shards written in local_id order:

    knowledge_base/<kb>/records/records_XXXXX.parquet
        schema: local_id(int64), query_id(int64), query(str), answer(str),
                query_type(str), source_lang(str), target_lang(str),
                english_passages(list[str]), translated_passages(list[str]),
                is_selected(list[int8])

This module loads those shards memory-mapped at backend startup and answers
local_id -> record lookups in well under 1 ms (bisect over shard ranges +
O(1) row indexing inside the shard).
"""
...
class RecordStore:
    """Memory-mapped parquet record store keyed by FAISS vector id (= local_id)."""
    ...
    def iter_queries(self, limit: int = 200, unique_only: bool = True) -> Iterator[Dict[str, Any]]:
        """Yields {local_id, query} in dataset order — used to build eval sets."""
        if not self.available:
            return
        seen = set()
        yielded = 0
        for idx in range(len(self._shard_paths)):
            ...
            base = self._shard_mins[idx]
            for off, q in enumerate(qs):
                if unique_only:
                    if q in seen:
                        continue
                    seen.add(q)
                yield {"local_id": base + ids[off] if False else int(ids[off]), "query": (q or "").strip()}
                yielded += 1
                if yielded >= limit:
                    return


def load_record_store(kb_dir: str) -> RecordStore:
    """Loads the record store shipped next to a FAISS index (knowledge_base/<kb>/records)."""
    ...
    if os.path.exists(manifest):
        try:
            ...
            pass
    return RecordStore(records_dir)
