...
# Client-side pacing for low-RPM judge keys (e.g. org-wide 10 RPM limits):
# a process-wide minimum interval between ANY two judge calls, shared across
# both check threads, plus bounded retry-on-429 honoring Retry-After. Keeps
# sustained throughput under the limit instead of bursting into errors.
_JUDGE_MIN_INTERVAL_S = float(os.environ.get("EVAL_JUDGE_MIN_INTERVAL_S", "6.5"))
_JUDGE_MAX_RETRIES = int(os.environ.get("EVAL_JUDGE_MAX_RETRIES", "5"))
...
def _pace_judge_call() -> None:
    ...
    with _pace_lock:
        ...
        if wait > 0:
            ...
            wait = 0.0
        _pace_last_call = time.monotonic()
...
def _call_anthropic(system_prompt: str, user_content: str) -> JudgeVerdict:
    ...
    try:
        ...
        response = _retry_on_rate_limit(_create)
